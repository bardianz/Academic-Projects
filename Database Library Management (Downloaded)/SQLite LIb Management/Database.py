...
def DatabaseCreate():
    cnx = mysql.connector.connect(user='root', password=mysqlpassword, host='localhost')
    Cursor = cnx.cursor()
...
def TablesCreate():
    cnx = mysql.connector.connect(user='root', password=mysqlpassword, host='localhost', database='Library')
    Cursor = cnx.cursor()
